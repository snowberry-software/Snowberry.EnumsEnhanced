...
```bash
# Run everything
dotnet run -c Release --project Tests/Snowberry.EnumsEnhanced.Benchmark -- --filter *

# Run a single class
dotnet run -c Release --project Tests/Snowberry.EnumsEnhanced.Benchmark -- --filter *HasFlagBenchmark*

# Run one or more categories
dotnet run -c Release --project Tests/Snowberry.EnumsEnhanced.Benchmark -- --anyCategories ToString ParseName

# List the available benchmarks without running them
dotnet run -c Release --project Tests/Snowberry.EnumsEnhanced.Benchmark -- --list flat
```
